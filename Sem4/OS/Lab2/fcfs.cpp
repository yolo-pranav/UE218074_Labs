/*
 * 25/01/2023
 * First Cum First Serve CPU Scheduling Algorithm with sorting.
 * Requirements: The processes can be added in any order irrespective of arrival times.
 */
...
// Swap the elements 'a' & 'b' passed in the parameter
...
// Sort the array of processes according their arrival time
void sort(process arr[], int size) {
    int i, j, imin;
    for(i = 0; i<size-1; i++) {
        imin = i;  
        for(j = i+1; j<size; j++)
            if(arr[j].at < arr[imin].at)
                imin = j;
            swap(arr[i], arr[imin]);
    }
}
